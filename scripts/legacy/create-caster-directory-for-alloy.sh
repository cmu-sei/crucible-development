#!/bin/bash
# Create a Caster Directory for Alloy (without creating a Player View)
# Use this when you already have a Player View Template

set -e

PROXMOX_HOST="${PROXMOX_HOST}"
PROXMOX_API_TOKEN="${PROXMOX_API_TOKEN}"
CASTER_API_URL="${CASTER_API_URL:-http://localhost:4309/api}"
KEYCLOAK_URL="${KEYCLOAK_URL:-https://localhost:8443}"
KEYCLOAK_USER="${KEYCLOAK_USER:-admin}"
KEYCLOAK_PASSWORD="${KEYCLOAK_PASSWORD:-admin}"
PROJECT_NAME="${PROJECT_NAME:-Alloy Proxmox Infrastructure}"
DIRECTORY_NAME="${DIRECTORY_NAME:-Proxmox VMs}"

echo "Creating Caster Directory for Alloy"
echo ""

# Show current environment variables
if [ -n "$PLAYER_VIEW_ID" ]; then
  echo "Current settings:"
  echo "  PLAYER_VIEW_ID: $PLAYER_VIEW_ID"
  echo ""
fi

# Check for required variables
if [ -z "$PROXMOX_HOST" ] || [ -z "$PROXMOX_API_TOKEN" ]; then
  echo "Error: PROXMOX_HOST and PROXMOX_API_TOKEN environment variables are required"
  echo ""
  echo "IMPORTANT: Use single quotes to prevent bash ! expansion"
  echo ""
  echo "Export variables:"
  echo "  export PROXMOX_HOST='your-proxmox-ip'"
  echo "  export PROXMOX_API_TOKEN='root@pam!crucible=your-token-here'"
  echo ""
  echo "Then run this script:"
  echo "  ./scripts/create-caster-directory-for-alloy.sh"
  exit 1
fi

echo "  Proxmox Host: $PROXMOX_HOST"
echo "  Caster API: $CASTER_API_URL"
echo "  Project: $PROJECT_NAME"
echo "  Directory: $DIRECTORY_NAME"
echo ""

# Get auth token
echo "Obtaining auth token..."
TOKEN_RESPONSE=$(curl -k -s -X POST "$KEYCLOAK_URL/realms/crucible/protocol/openid-connect/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "client_id=caster.ui" \
  -d "grant_type=password" \
  -d "username=$KEYCLOAK_USER" \
  -d "password=$KEYCLOAK_PASSWORD")

ACCESS_TOKEN=$(echo $TOKEN_RESPONSE | grep -o '"access_token":"[^"]*' | cut -d'"' -f4)

if [ -z "$ACCESS_TOKEN" ]; then
  echo "✗ Failed to obtain token"
  exit 1
fi
echo "✓ Token obtained"
echo ""

# Find next available project name
echo "Finding available project name..."
ALL_PROJECTS=$(curl -k -s -m 10 -X GET "$CASTER_API_URL/projects" \
  -H "Authorization: Bearer $ACCESS_TOKEN")

# Check if base name exists
EXISTING_PROJECT=$(echo "$ALL_PROJECTS" | jq -r ".[] | select(.name == \"$PROJECT_NAME\") | .name" | head -1)

if [ -n "$EXISTING_PROJECT" ] && [ "$EXISTING_PROJECT" != "null" ]; then
  # Find highest numbered project
  COUNTER=2
  while true; do
    TEST_NAME="${PROJECT_NAME} ${COUNTER}"
    EXISTS=$(echo "$ALL_PROJECTS" | jq -r ".[] | select(.name == \"$TEST_NAME\") | .name" | head -1)
    if [ -z "$EXISTS" ] || [ "$EXISTS" = "null" ]; then
      PROJECT_NAME="$TEST_NAME"
      break
    fi
    COUNTER=$((COUNTER + 1))
  done
  echo "✓ Using name: $PROJECT_NAME"
fi

# Create new project
echo "Creating project..."
PROJECT_ID=$(cat /proc/sys/kernel/random/uuid)
PROJECT_RESPONSE=$(curl -k -s -X POST "$CASTER_API_URL/projects" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"id\": \"$PROJECT_ID\",
    \"name\": \"$PROJECT_NAME\",
    \"description\": \"Infrastructure project for Alloy events\"
  }")

if echo "$PROJECT_RESPONSE" | jq -e '.id' > /dev/null 2>&1; then
  PROJECT_ID=$(echo "$PROJECT_RESPONSE" | jq -r '.id')
  echo "✓ Project created: $PROJECT_ID"
else
  echo "✗ Failed to create project"
  echo "$PROJECT_RESPONSE"
  exit 1
fi
echo ""


# Create Directory
echo "Creating directory..."
DIRECTORY_ID=$(cat /proc/sys/kernel/random/uuid)
DIRECTORY_RESPONSE=$(curl -k -s -w "\n%{http_code}" -X POST "$CASTER_API_URL/directories" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"id\": \"$DIRECTORY_ID\",
    \"projectId\": \"$PROJECT_ID\",
    \"name\": \"$DIRECTORY_NAME\",
    \"terraformVersion\": \"1.5.0\"
  }")

HTTP_CODE=$(echo "$DIRECTORY_RESPONSE" | tail -n1)
RESPONSE=$(echo "$DIRECTORY_RESPONSE" | head -n-1)

if [ "$HTTP_CODE" = "201" ] || echo "$RESPONSE" | grep -q '"id"'; then
  DIRECTORY_ID=$(echo "$RESPONSE" | jq -r '.id' 2>/dev/null || echo "$DIRECTORY_ID")
  echo "✓ Directory created: $DIRECTORY_ID"
else
  echo "✗ Failed to create directory"
  echo "$RESPONSE"
  exit 1
fi
echo ""

# Create main.tf file with Proxmox and Crucible providers
echo "Creating Terraform configuration..."
MAIN_TF='terraform {
  required_providers {
    proxmox = {
      source  = "bpg/proxmox"
      version = "0.106.0"
    }
    crucible = {
      source  = "cmu-sei/crucible"
      version = "~> 2.5"
    }
  }
}

provider "proxmox" {
  endpoint = var.proxmox_endpoint
  api_token = var.proxmox_api_token
  insecure = var.proxmox_insecure
}

provider "crucible" {
  username       = var.crucible_username
  password       = var.crucible_password
  auth_url       = var.crucible_auth_url
  token_url      = var.crucible_token_url
  client_id      = var.crucible_client_id
  client_secret  = var.crucible_client_secret
  client_scopes  = ["player", "player-vm"]
  vm_api_url     = var.vm_api_url
  player_api_url = var.player_api_url
  caster_api_url = var.caster_api_url
}

# Alloy automatically injects team variables based on team names
# For a view with "Admin" team, Alloy creates: admin = "team-uuid"
# We use the admin team variable that Alloy provides
locals {
  # Alloy injects team IDs as variables - use admin team by default
  team_ids = [var.admin]
}

# Create VMs in Proxmox using bpg provider
resource "proxmox_virtual_environment_vm" "alpine" {
  name      = "alpine-alloy-${var.instance_suffix}"
  node_name = "pve"

  clone {
    vm_id = 105
  }

  cpu {
    cores = 1
  }

  memory {
    dedicated = 512
  }
}

resource "proxmox_virtual_environment_vm" "tinycore" {
  name      = "tinycore-alloy-${var.instance_suffix}"
  node_name = "pve"

  clone {
    vm_id = 102
  }

  cpu {
    cores = 1
  }

  memory {
    dedicated = 512
  }
}

# Register VMs in Player VM API to all teams in the view
resource "crucible_player_virtual_machine" "alpine" {
  name       = proxmox_virtual_environment_vm.alpine.name
  team_ids   = local.team_ids
  embeddable = true

  proxmox_vm_info {
    id   = proxmox_virtual_environment_vm.alpine.vm_id
    node = "pve"
  }

}

resource "crucible_player_virtual_machine" "tinycore" {
  name       = proxmox_virtual_environment_vm.tinycore.name
  team_ids   = local.team_ids
  embeddable = true

  proxmox_vm_info {
    id   = proxmox_virtual_environment_vm.tinycore.vm_id
    node = "pve"
  }

}

output "view_id" {
  value = var.view_id
}

output "team_ids" {
  value = local.team_ids
}

output "alpine_vmid" {
  value = proxmox_virtual_environment_vm.alpine.vm_id
}

output "alpine_name" {
  value = proxmox_virtual_environment_vm.alpine.name
}

output "alpine_player_id" {
  value = crucible_player_virtual_machine.alpine.id
}

output "tinycore_vmid" {
  value = proxmox_virtual_environment_vm.tinycore.vm_id
}

output "tinycore_name" {
  value = proxmox_virtual_environment_vm.tinycore.name
}

output "tinycore_player_id" {
  value = crucible_player_virtual_machine.tinycore.id
}'

FILE_ID=$(cat /proc/sys/kernel/random/uuid)
FILE_RESPONSE=$(curl -k -s -w "\n%{http_code}" -X POST "$CASTER_API_URL/files" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"id\": \"$FILE_ID\",
    \"directoryId\": \"$DIRECTORY_ID\",
    \"name\": \"main.tf\",
    \"content\": $(echo "$MAIN_TF" | jq -Rs .)
  }")

HTTP_CODE=$(echo "$FILE_RESPONSE" | tail -n1)

if [ "$HTTP_CODE" = "201" ]; then
  echo "✓ Created main.tf"
else
  echo "✗ Failed to create main.tf"
  exit 1
fi

# Create variables.tf
VARIABLES_TF='variable "proxmox_endpoint" {
  description = "Proxmox API endpoint"
  type        = string
}

variable "proxmox_api_token" {
  description = "Proxmox API token"
  type        = string
  sensitive   = true
}

variable "proxmox_insecure" {
  description = "Skip TLS verification"
  type        = bool
  default     = true
}

variable "instance_suffix" {
  description = "Suffix for VM names to ensure uniqueness"
  type        = string
  default     = "1"
}

variable "view_id" {
  description = "Player View ID created by Alloy"
  type        = string
}

variable "admin" {
  description = "Admin team ID (injected by Alloy)"
  type        = string
}

variable "crucible_username" {
  description = "Crucible username"
  type        = string
}

variable "crucible_password" {
  description = "Crucible password"
  type        = string
  sensitive   = true
}

variable "crucible_auth_url" {
  description = "Crucible auth URL"
  type        = string
}

variable "crucible_token_url" {
  description = "Crucible token URL"
  type        = string
}

variable "crucible_client_id" {
  description = "Crucible OAuth client ID"
  type        = string
  default     = "crucible.provider"
}

variable "crucible_client_secret" {
  description = "Crucible OAuth client secret"
  type        = string
  default     = ""
}

variable "player_api_url" {
  description = "Player API URL"
  type        = string
  default     = "http://host.docker.internal:4300/api"
}

variable "vm_api_url" {
  description = "VM API URL"
  type        = string
  default     = "http://host.docker.internal:4302/api"
}

variable "caster_api_url" {
  description = "Caster API URL"
  type        = string
  default     = "http://localhost:4309"
}'

FILE_ID=$(cat /proc/sys/kernel/random/uuid)
FILE_RESPONSE=$(curl -k -s -w "\n%{http_code}" -X POST "$CASTER_API_URL/files" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"id\": \"$FILE_ID\",
    \"directoryId\": \"$DIRECTORY_ID\",
    \"name\": \"variables.tf\",
    \"content\": $(echo "$VARIABLES_TF" | jq -Rs .)
  }")

HTTP_CODE=$(echo "$FILE_RESPONSE" | tail -n1)

if [ "$HTTP_CODE" = "201" ]; then
  echo "✓ Created variables.tf"
else
  echo "✗ Failed to create variables.tf"
  exit 1
fi

# Create terraform.tfvars
TERRAFORM_TFVARS="proxmox_endpoint = \"https://${PROXMOX_HOST}:8006\"
proxmox_api_token = \"${PROXMOX_API_TOKEN}\"
proxmox_insecure = true
instance_suffix = \"1\"

# Crucible provider config - Alloy will override view_id at runtime
view_id = \"PLACEHOLDER\"
crucible_username = \"admin\"
crucible_password = \"admin\"
crucible_auth_url = \"https://host.docker.internal:8443/realms/crucible/protocol/openid-connect/auth\"
crucible_token_url = \"https://host.docker.internal:8443/realms/crucible/protocol/openid-connect/token\"
crucible_client_id = \"crucible.provider\"
crucible_client_secret = \"\"

player_api_url = \"http://host.docker.internal:4300/api\"
vm_api_url = \"http://host.docker.internal:4302/api\"
caster_api_url = \"http://localhost:4309\"
"

FILE_ID=$(cat /proc/sys/kernel/random/uuid)
FILE_RESPONSE=$(curl -k -s -w "\n%{http_code}" -X POST "$CASTER_API_URL/files" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"id\": \"$FILE_ID\",
    \"directoryId\": \"$DIRECTORY_ID\",
    \"name\": \"terraform.tfvars\",
    \"content\": $(echo "$TERRAFORM_TFVARS" | jq -Rs .)
  }")

HTTP_CODE=$(echo "$FILE_RESPONSE" | tail -n1)

if [ "$HTTP_CODE" = "201" ]; then
  echo "✓ Created terraform.tfvars"
else
  echo "✗ Failed to create terraform.tfvars"
  exit 1
fi

echo ""
echo "✓ Caster directory created successfully!"
echo ""
echo "Project: $PROJECT_NAME ($PROJECT_ID)"
echo "Directory: $DIRECTORY_NAME ($DIRECTORY_ID)"
echo ""
echo "This directory creates:"
echo "  - 2 Proxmox VMs (alpine, tinycore)"
echo "  - Registers VMs with all teams in the Player View"
echo "  - Uses Crucible provider to access view data"
echo ""
echo "How it works:"
echo "  - Alloy clones the view template when launching an event"
echo "  - Alloy automatically injects view_id variable into Terraform"
echo "  - Terraform queries the view and registers VMs to all teams"
echo ""
echo "Next steps:"
echo "  1. Use this directory with your Player View Template for Alloy"
echo "  2. Access Caster UI: http://localhost:4310"
echo ""
echo "Paste these commands to create Alloy event template:"
echo ""
echo "export CASTER_DIRECTORY_ID='$DIRECTORY_ID'"
if [ -n "$PLAYER_VIEW_ID" ]; then
  echo "export PLAYER_VIEW_ID='$PLAYER_VIEW_ID'"
else
  echo "export PLAYER_VIEW_ID='your-player-view-template-id'"
fi
echo "./scripts/create-alloy-event.sh"
echo ""
