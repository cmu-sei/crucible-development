...
echo "Checking for existing View..."
VIEW_CHECK=$(curl -k -s -X GET "$PLAYER_API_URL/views/$VIEW_ID" \
  -H "Authorization: Bearer $ACCESS_TOKEN" 2>&1 || true)

if echo "$VIEW_CHECK" | grep -q '"id"'; then
  echo "✓ View already exists"
else
  echo "Creating View..."
  VIEW_RESPONSE=$(timeout 5 curl -k -s -X POST "$PLAYER_API_URL/views" \
  ...
  if echo "$VIEW_RESPONSE" | grep -q '"id"'; then
    echo "✓ View created"
  else
...
# Check if Virtual Machines application is already in the View
echo "Checking if Virtual Machines application is in View..."
VIEW_APPS=$(curl -k -s -X GET "$PLAYER_API_URL/views/$VIEW_ID/applications" \
  -H "Authorization: Bearer $ACCESS_TOKEN" 2>&1 || true)

APP_EXISTS=$(echo "$VIEW_APPS" | grep -o "\"applicationTemplateId\":\"$APP_TEMPLATE_ID\"" || true)

if [ -n "$APP_EXISTS" ]; then
  echo "✓ Virtual Machines application already in View"
else
  echo "Adding Virtual Machines application to View..."
  APP_RESPONSE=$(timeout 5 curl -k -s -X POST "$PLAYER_API_URL/views/$VIEW_ID/applications" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
    -d "{
      \"viewId\": \"$VIEW_ID\",
  ...
  elif echo "$APP_RESPONSE" | grep -q "409\|duplicate"; then
    echo "✓ Application already in view"
  else
    echo "⚠ Could not add application (may already exist)"
  fi
fi
echo ""

# Check if Team exists
echo "Checking for existing Team..."
TEAM_CHECK=$(curl -k -s -X GET "$PLAYER_API_URL/teams/$TEAM_ID" \
  -H "Authorization: Bearer $ACCESS_TOKEN" 2>&1 || true)

if echo "$TEAM_CHECK" | grep -q '"id"'; then
  echo "✓ Team already exists"
else
  echo "Creating Team..."
  TEAM_RESPONSE=$(timeout 5 curl -k -s -X POST "$PLAYER_API_URL/teams" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
    -d "{
      \"id\": \"$TEAM_ID\",
...
# Check if VM with this Proxmox ID already exists
echo "Checking for existing VM with Proxmox ID $PROXMOX_VM_ID..."
...
if command -v jq &> /dev/null; then
  EXISTING_VM_ID=$(echo "$ALL_VMS" | jq -r ".[] | select(.proxmoxVmInfo.id == $PROXMOX_VM_ID) | .id" | head -1)
else
  EXISTING_VM=$(echo "$ALL_VMS" | grep -o "\"proxmoxVmInfo\":{\"id\":$PROXMOX_VM_ID" || true)
  if [ -n "$EXISTING_VM" ]; then
    EXISTING_VM_ID=$(echo "$ALL_VMS" | grep -B50 "\"proxmoxVmInfo\":{\"id\":$PROXMOX_VM_ID" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
  fi
fi

if [ -n "$EXISTING_VM_ID" ]; then
  echo "✓ VM with Proxmox ID $PROXMOX_VM_ID already exists (UUID: $EXISTING_VM_ID)"
  echo ""
  echo "VM Details:"
  echo "  VM API ID: $EXISTING_VM_ID"
  echo "  Proxmox VM ID: $PROXMOX_VM_ID"
  echo "  Team: $TEAM_ID"
  echo ""
  echo "Access VM at:"
