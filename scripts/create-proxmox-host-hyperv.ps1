...
if ($existingVM) {
    ...
    Write-Host ""
    Write-Host "To remove existing VM and files:" -ForegroundColor Yellow
    Write-Host "  Stop-VM -Name '$VMName' -TurnOff -ErrorAction SilentlyContinue"
    Write-Host "  Remove-VM -Name '$VMName' -Force"
    Write-Host "  Remove-Item -Path '$vmDir' -Recurse -Force"
    exit 1
}

# Check if VM directory already exists (from previous failed run)
if (Test-Path $vmDir) {
    Write-Host "Warning: VM directory already exists: $vmDir" -ForegroundColor Yellow
    Write-Host "This may be from a previous failed run." -ForegroundColor Yellow
    Write-Host ""
    $response = Read-Host "Delete existing directory and continue? (y/N)"
...
# Create VM directory (should be clean at this point)
...
Write-Host "     (Token script will show command to update appsettings automatically)"
Write-Host "  9. Run: PROXMOX_HOST=<PROXMOX_IP> ./scripts/setup-proxmox-nginx.sh"
Write-Host ""
