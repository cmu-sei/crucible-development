...
map_plugin_to_container_path() {
    ...
    case "$plugin_type" in
        ...
        gradereport) echo "/var/www/html/grade/report/$plugin_subdir" ;;
...
// Moodle 5.1+ serves from /var/www/html/public, and this one file is baked into the
// image for every version Aspire runs. Add the public/ spelling of each path so the
// filter matches either layout; a path that does not exist is simply never matched.
foreach ($includePaths as $path) {
    if (strpos($path, "/var/www/html/public/") !== 0) {
        $includePaths[] = str_replace("/var/www/html/", "/var/www/html/public/", $path);
    }
}
