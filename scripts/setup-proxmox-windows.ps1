...
function Create-ProxmoxVM {
    ...
    if ([string]::IsNullOrEmpty($ProxmoxISO)) {
        $repoRoot = Split-Path $PSScriptRoot -Parent
        $proxmoxIsos = @(Get-ChildItem -Path $repoRoot -Filter "proxmox-ve_*.iso" -File)

        if ($proxmoxIsos.Count -eq 0) {
            ...
            Write-Host "  .\scripts\setup-proxmox-windows.ps1 -ProxmoxISO 'C:\path\to\proxmox-ve_9.2-1.iso'"
            exit 1
        }

        if ($proxmoxIsos.Count -gt 1) {
            Write-Host "Error: Multiple Proxmox installer ISOs were found in the repository root." -ForegroundColor Red
            $proxmoxIsos | ForEach-Object { Write-Host "  $($_.Name)" -ForegroundColor Yellow }
            Write-Host ""
            Write-Host "Specify the intended installer explicitly with -ProxmoxISO." -ForegroundColor Yellow
            exit 1
        }

        $ProxmoxISO = $proxmoxIsos[0].FullName
    }
