#!/bin/bash
# Create Alpine Linux VM on Proxmox host
...
echo "Creating Alpine Linux VM on Proxmox"
...
qm create $VM_ID \
  ...
  --ide2 local:iso/${ALPINE_ISO},media=cdrom \
  --boot order=ide2 \
  --ostype l26 \
  --scsihw virtio-scsi-pci \
  --scsi0 local-lvm:4,format=raw \
  --vga std
EOF

echo ""
echo "✓ Alpine Linux VM created successfully!"
echo ""
echo "VM Details:"
echo "  VM ID: $VM_ID"
echo "  Name: $VM_NAME"
