...
DEFAULT_CONFIG_FILE="$REPO_ROOT/Crucible.AppHost/resources/proxmox/config"
LEGACY_CONFIG_FILE="$HOME/.crucible-proxmox"
CONFIG_FILE="${PROXMOX_CONFIG_FILE:-$DEFAULT_CONFIG_FILE}"
...
declare -A RESOURCE_IDS=(
    ...
    [alloy_event_task_grading]="74257398-e6ed-46e3-881a-8d7edacfddb9"
)
...
STEAMFITTER_API_URL="${STEAMFITTER_API_URL:-http://localhost:4400/api}"

# Caster runs Terraform in Kubernetes; from that job, localhost is the job
# container, not the dev container running Aspire services.
TERRAFORM_CRUCIBLE_HOST="${TERRAFORM_CRUCIBLE_HOST:-host.docker.internal}"
TERRAFORM_KEYCLOAK_URL="${TERRAFORM_KEYCLOAK_URL:-https://${TERRAFORM_CRUCIBLE_HOST}:8443}"
TERRAFORM_PLAYER_API_URL="${TERRAFORM_PLAYER_API_URL:-http://${TERRAFORM_CRUCIBLE_HOST}:4300/api}"
TERRAFORM_VM_API_URL="${TERRAFORM_VM_API_URL:-http://${TERRAFORM_CRUCIBLE_HOST}:4302/api}"
TERRAFORM_CASTER_API_URL="${TERRAFORM_CASTER_API_URL:-http://${TERRAFORM_CRUCIBLE_HOST}:4309/api}"
...
# Load config file
load_config() {
    local candidates=("$CONFIG_FILE")

    # Keep existing developer environments working, but prefer the repo-local
    # ignored config path above.
    if [ -z "${PROXMOX_CONFIG_FILE:-}" ] && [ "$LEGACY_CONFIG_FILE" != "$CONFIG_FILE" ]; then
...
if [ -z "$pve_version" ]; then
    echo "Could not determine the installed Proxmox VE version." >&2
    exit 1
fi

pve_major="${pve_version%%.*}"
if [ "$pve_major" -lt 9 ]; then
    echo "Proxmox VE $pve_version is unsupported; this setup requires Proxmox VE 9 or later." >&2
    exit 1
fi
...
CHECKEOF
        ...
        log_error "Proxmox preflight failed."
        echo "$output" >&2
        return 1
    fi
...
save_config() {
    ...
    {
        echo "# Crucible Proxmox Configuration"
        echo "# Auto-generated by crucible-proxmox.sh on $(date -Iseconds)"
        printf "export PROXMOX_HOST=%q\n" "$PROXMOX_HOST"
        printf "export PROXMOX_API_TOKEN=%q\n" "$PROXMOX_API_TOKEN"
        if [ -n "$KEYCLOAK_HOST" ]; then
            printf "export KEYCLOAK_HOST=%q\n" "$KEYCLOAK_HOST"
        fi
        printf "LAST_SETUP_DATE=%q\n" "$(date -Iseconds)"
...
# The Proxmox agent flag only creates the virtio channel; the guest still needs
# qemu-guest-agent installed and a serial getty for browser console access.
mkdir -p /var/lib/vz/snippets
LOCAL_CONTENT="$(awk '
    $1 == "dir:" && $2 == "local" { in_local = 1; next }
    /^[^[:space:]]/ { in_local = 0 }
    in_local && $1 == "content" { print $2 }
' /etc/pve/storage.cfg)"
if [ -z "$LOCAL_CONTENT" ]; then
    LOCAL_CONTENT="backup,iso,vztmpl"
fi
if ! echo ",${LOCAL_CONTENT}," | grep -q ",snippets,"; then
    pvesm set local --content "${LOCAL_CONTENT},snippets"
fi

cat > /var/lib/vz/snippets/crucible-alpine-user-data.yaml <<'CLOUDINIT'
#cloud-config
package_update: true
packages:
  - qemu-guest-agent
...
runcmd:
  ...
  - [ sh, -c, "rc-service qemu-guest-agent start || service qemu-guest-agent start || true" ]
  - [ sh, -c, "sed -i '/^#*ttyS0::/d' /etc/inittab; echo 'ttyS0::respawn:/sbin/getty -L ttyS0 115200 vt100' >> /etc/inittab; kill -HUP 1 || true" ]
  - [ sh, -c, "echo 'Alpine template ready on ttyS0' > /dev/ttyS0 || true" ]
CLOUDINIT
...
qm set ${TEMPLATE_ID} --efidisk0 local-lvm:0,efitype=4m,pre-enrolled-keys=0
qm set ${TEMPLATE_ID} --boot c --bootdisk scsi0
qm set ${TEMPLATE_ID} --agent enabled=1
qm set ${TEMPLATE_ID} --ide2 local-lvm:cloudinit
qm set ${TEMPLATE_ID} --cicustom user=local:snippets/crucible-alpine-user-data.yaml
...
create_topomojo_templates() {
    ...
    if [ -n "$target_template_name" ]; then
        # Filter by name pattern, exclude published templates (isPublished might be missing or true)
        local existing_ids=$(echo "$all_templates" | jq -r ".[] | select(.name | test(\"^${target_template_name}(-[0-9]+)?\$\")) | select(.isPublished == false or .isPublished == null) | .id" 2>/dev/null || echo "")
...
upsert_caster_file() {
    ...
    local actual_file_id=""
    local existing_files=$(curl -k -s -X GET "$CASTER_API_URL/directories/$directory_id/files" \
        -H "Authorization: Bearer $token" 2>/dev/null)

    actual_file_id=$(echo "$existing_files" | jq -r --arg file_name "$file_name" '.[]? | select(.name == $file_name) | .id' | head -1)

    if [ -z "$actual_file_id" ] || [ "$actual_file_id" = "null" ]; then
        local existing_file=$(curl -k -s -X GET "$CASTER_API_URL/files/$file_id" \
            -H "Authorization: Bearer $token" 2>/dev/null)
        actual_file_id=$(echo "$existing_file" | jq -r '.id // empty')
    fi

    local method="POST"
    local url="$CASTER_API_URL/files"
    local success_message="Created $file_name"
    local payload_file_id="$file_id"

    if [ -n "$actual_file_id" ] && [ "$actual_file_id" != "null" ]; then
        method="PUT"
        url="$CASTER_API_URL/files/$actual_file_id"
        payload_file_id="$actual_file_id"
        success_message="Updated $file_name"

        local lock_response=$(curl -k -s -w "\n%{http_code}" -X POST "$CASTER_API_URL/files/$actual_file_id/actions/lock" \
            -H "Authorization: Bearer $token" \
            -H "Content-Type: application/json" \
            -d "{}")
        local lock_http_code=$(echo "$lock_response" | tail -1)
        local lock_body=$(echo "$lock_response" | sed '$d')

        if [ "$lock_http_code" != "200" ]; then
            log_error "Failed to lock $file_name (HTTP $lock_http_code): $lock_body"
            return 1
        fi
    fi
    ...
    if [ "$method" = "PUT" ]; then
        curl -k -s -X POST "$CASTER_API_URL/files/$actual_file_id/actions/unlock" \
            -H "Authorization: Bearer $token" \
            -H "Content-Type: application/json" \
            -d "{}" > /dev/null 2>&1 || true
    fi
...
create_caster_project() {
    ...
    log_info "Creating/updating Terraform files in directory..."
...
locals {
  generated_name_suffix = substr(replace(coalesce(var.view_id, var.username), "/[^A-Za-z0-9-]/", "-"), 0, 32)

  player_vm_team_ids = [
    for team_id in [var.admin, var.admin_user] : team_id
    if team_id != null && team_id != ""
  ]
}

resource "proxmox_virtual_environment_vm" "tinycore" {
  name      = "tinycore-caster-${local.generated_name_suffix}"
  node_name = "pve"
...
resource "proxmox_virtual_environment_vm" "puppy" {
  name      = "puppy-caster-${local.generated_name_suffix}"
  node_name = "pve"
...
resource "crucible_player_virtual_machine" "tinycore" {
  ...
  team_ids   = local.player_vm_team_ids
  user_id    = var.user_id
  embeddable = true
...
resource "crucible_player_virtual_machine" "puppy" {
  ...
  proxmox_vm_info {
    ...
    upsert_caster_file "$token" "$main_tf_id" "$directory_id" "main.tf" "$main_tf_content"

    # Create variables.tf
    local variables_tf_content='variable "admin" {
  description = "Player Admin team ID injected by Alloy from the cloned view team named Admin."
  type        = string
  default     = null
}

variable "admin_user" {
  description = "Player team ID injected by Alloy when the cloned view team name is Admin User."
  type        = string
  default     = null
}
...
variable "exercise_id" {
  description = "Deprecated Alloy/Caster event view ID compatibility variable."
  type        = string
  default     = null
}
...
variable "user_id" {
  description = "Alloy event user ID."
  type        = string
  default     = null
}

variable "username" {
  description = "Alloy event username."
  type        = string
  default     = "user"
}

variable "view_id" {
  description = "Alloy event Player view ID."
  type        = string
  default     = null
}
...
variable "vm_api_url" {
  ...
  type        = string
    ...
    upsert_caster_file "$token" "$variables_tf_id" "$directory_id" "variables.tf" "$variables_tf_content"
...
crucible_auth_url = \"${TERRAFORM_KEYCLOAK_URL}\"
crucible_token_url = \"${TERRAFORM_KEYCLOAK_URL}/realms/crucible/protocol/openid-connect/token\"
crucible_client_id = \"player.vm.admin\"
crucible_client_secret = \"\"
vm_api_url = \"${TERRAFORM_VM_API_URL}\"
player_api_url = \"${TERRAFORM_PLAYER_API_URL}\"
caster_api_url = \"${TERRAFORM_CASTER_API_URL}\""

    upsert_caster_file "$token" "$tfvars_id" "$directory_id" "terraform.tfvars" "$tfvars_content"
...
register_player_vms() {
    ...
    local failures=0
    register_player_vm "${RESOURCE_IDS[puppy_vm]}" "puppy-test" "$PUPPY_PROXMOX_ID" "$admin_team_id" "$token" || failures=$((failures + 1))
    register_player_vm "${RESOURCE_IDS[alpine_vm]}" "alpine-linux-template" "$ALPINE_PROXMOX_ID" "$admin_team_id" "$token" || failures=$((failures + 1))
    register_player_vm "${RESOURCE_IDS[tinycore_vm]}" "tinycore-linux-template" "$TINYCORE_PROXMOX_ID" "$admin_team_id" "$token" || failures=$((failures + 1))
...
create_alloy_event_task_grading() {
    local event_name="Moodle Task Grading Test Event"
    local event_id="${RESOURCE_IDS[alloy_event_task_grading]}"
    local player_view_id="${RESOURCE_IDS[player_view_template]}"
    local caster_dir_id="${RESOURCE_IDS[caster_project2_dir]}"

    log_step "Creating Alloy event (Steamfitter task grading): $event_name"

    local alloy_token=$(get_keycloak_token "${KEYCLOAK_CLIENTS[alloy]}")
    local steamfitter_token=$(get_keycloak_token "${KEYCLOAK_CLIENTS[steamfitter]}")
    local scenario_template_id=$(curl -k -s -X GET "$STEAMFITTER_API_URL/scenarioTemplates" \
        -H "Authorization: Bearer $steamfitter_token" 2>/dev/null \
        | jq -r '.[]? | select(.name == "Moodle Task Grading Test Scenario") | .id' | head -1)

    if [ -z "$scenario_template_id" ] || [ "$scenario_template_id" = "null" ]; then
        log_error "Steamfitter scenario template not found; cannot create task-grading Alloy event"
        return 1
    fi

    local existing=$(curl -k -s -X GET "$ALLOY_API_URL/eventtemplates" \
        -H "Authorization: Bearer $alloy_token" 2>/dev/null \
        | jq -r --arg name "$event_name" '.[]? | select(.name == $name) | .id' | head -1)

    local payload=$(jq -n \
        --arg id "$event_id" \
        --arg name "$event_name" \
        --arg directoryId "$caster_dir_id" \
        --arg viewId "$player_view_id" \
        --arg scenarioTemplateId "$scenario_template_id" \
        '{
            id: $id,
            name: $name,
            description: "Event template for Moodle Crucible task grading coverage.",
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY RUN] Would create/update Alloy task-grading event"
        return 0
    fi

    local method="POST"
    local url="$ALLOY_API_URL/eventtemplates"
    if [ -n "$existing" ] && [ "$existing" != "null" ]; then
        method="PUT"
        url="$ALLOY_API_URL/eventtemplates/$existing"
        payload=$(echo "$payload" | jq --arg id "$existing" '.id = $id')
    fi
    ...
    if [ "$http_code" = "200" ] || [ "$http_code" = "201" ]; then
        local created_id=$(echo "$body" | jq -r '.id // empty')
        log_success "Alloy task-grading event ready: ${created_id:-$existing}"
        return 0
    fi

    log_error "Failed to create/update Alloy task-grading event (HTTP $http_code): ${body:0:300}"
    return 1
}

# ============================================================
# STEAMFITTER FUNCTIONS
# ============================================================

create_steamfitter_task_if_missing() {
    local token="$1"
    local scenario_template_id="$2"
    local existing_tasks="$3"
    local task_name="$4"
    local task_description="$5"
    local expected_output="$6"
    local score="$7"
    local command_output="${8:-$expected_output}"

    local existing_id=$(echo "$existing_tasks" | jq -r --arg name "$task_name" '.[]? | select(.name == $name) | .id' | head -1)
    if [ -n "$existing_id" ] && [ "$existing_id" != "null" ]; then
        log_success "Steamfitter task already exists: $task_name"
        return 0
    fi

    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY RUN] Would create Steamfitter task: $task_name"
        return 0
    fi
    ...
    local payload=$(jq -n \
        ...
        '{
            ...
            actionParameters: {
                CommandText: "sh",
                CommandArgs: ("-lc \"printf %s " + $commandOutput + "\""),
                Username: "root",
                Password: "password"
            },
    ...
    local response=$(curl -k -s -w "\n%{http_code}" -X POST "$STEAMFITTER_API_URL/Tasks" \
        -H "Authorization: Bearer $token" \
        -H "Content-Type: application/json" \
        -d "$payload")
    local http_code=$(echo "$response" | tail -1)
    ...
    if [ "$http_code" = "200" ] || [ "$http_code" = "201" ]; then
        log_success "Created Steamfitter task: $task_name"
        return 0
    fi

    log_error "Failed to create Steamfitter task $task_name (HTTP $http_code): ${body:0:300}"
    return 1
}

create_steamfitter_grading_tasks() {
    local template_name="Moodle Task Grading Test Scenario"

    log_step "Creating default Steamfitter tasks for Moodle grading tests"

    local token=$(get_keycloak_token "${KEYCLOAK_CLIENTS[steamfitter]}")
    if [ -z "$token" ]; then
        log_error "Failed to get Steamfitter token"
        return 1
    fi

    local templates=$(curl -k -s -X GET "$STEAMFITTER_API_URL/scenarioTemplates" \
        -H "Authorization: Bearer $token" 2>/dev/null || echo "[]")
    local scenario_template_id=$(echo "$templates" | jq -r --arg name "$template_name" '.[]? | select(.name == $name) | .id' | head -1)

    if [ -z "$scenario_template_id" ] || [ "$scenario_template_id" = "null" ]; then
        if [ "$DRY_RUN" = "true" ]; then
            log_info "[DRY RUN] Would create Steamfitter scenario template: $template_name"
            return 0
        fi
        ...
        local template_payload=$(jq -n \
            ...
            '{
                name: $name,
                description: "Default scenario template for Moodle Crucible task grading tests.",
                durationHours: 2,
                vmCredentials: []
            }')
        local response=$(curl -k -s -w "\n%{http_code}" -X POST "$STEAMFITTER_API_URL/scenarioTemplates" \
            -H "Authorization: Bearer $token" \
            -H "Content-Type: application/json" \
            -d "$template_payload")
        local http_code=$(echo "$response" | tail -1)
        ...
        if [ "$http_code" != "200" ] && [ "$http_code" != "201" ]; then
            log_error "Failed to create Steamfitter scenario template (HTTP $http_code): ${body:0:300}"
            return 1
        fi

        scenario_template_id=$(echo "$body" | jq -r '.id')
        log_success "Created Steamfitter scenario template: $scenario_template_id"
    else
        log_success "Steamfitter scenario template already exists: $scenario_template_id"
    fi

    local existing_tasks=$(curl -k -s -X GET "$STEAMFITTER_API_URL/scenarioTemplates/$scenario_template_id/Tasks" \
        -H "Authorization: Bearer $token" 2>/dev/null || echo "[]")

    create_steamfitter_task_if_missing "$token" "$scenario_template_id" "$existing_tasks" \
        "Moodle grading complete" \
        "Deterministic passing task for full-credit Moodle grading checks." \
        "MOODLE_TASK_COMPLETE" \
        50 || return 1

    create_steamfitter_task_if_missing "$token" "$scenario_template_id" "$existing_tasks" \
        "Moodle grading partial" \
        "Second passing task used with incomplete result sets for partial-credit checks." \
        "MOODLE_TASK_PARTIAL" \
        25 || return 1

    create_steamfitter_task_if_missing "$token" "$scenario_template_id" "$existing_tasks" \
        "Moodle grading expected failure" \
        "Deterministic task with mismatched output for failed-result grading checks." \
        "MOODLE_TASK_EXPECTED_FAILURE" \
        25 \
        "MOODLE_TASK_ACTUAL_FAILURE" || return 1

    log_success "Steamfitter Moodle grading tasks ready"
}
...
create_moodle_task_grading_lab() {
    local course_id="${MOODLE_TASK_GRADING_COURSE_ID:-2}"
    local section_num="${MOODLE_TASK_GRADING_SECTION:-1}"
    local event_template_id="${RESOURCE_IDS[alloy_event_task_grading]}"

    log_step "Creating Moodle Crucible lab for task grading tests"

    if ! docker ps --format '{{.Names}}' | grep -qx moodle; then
        log_error "Moodle container is not running; cannot create Moodle task-grading lab"
        return 1
    fi
    ...
    if [ -z "$scenario_template_id" ] || [ "$scenario_template_id" = "null" ]; then
        log_error "Steamfitter scenario template not found; cannot create Moodle task-grading lab"
        return 1
    fi

    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY RUN] Would create Moodle Crucible task-grading lab"
        return 0
    fi

    docker exec -i moodle php <<PHP
<?php
...
\$name = 'Moodle Task Grading Test Event';

\$course = \$DB->get_record('course', ['id' => \$courseid], '*', MUST_EXIST);
\$module = \$DB->get_record('modules', ['name' => 'crucible'], '*', MUST_EXIST);
\$crucible = \$DB->get_record('crucible', ['eventtemplateid' => \$eventtemplateid]);

if (\$crucible) {
    \$cm = get_coursemodule_from_instance('crucible', \$crucible->id, \$courseid, false, MUST_EXIST);
    echo "Crucible Moodle activity already exists: crucibleid={\$crucible->id} cmid={\$cm->id}\n";
} else {
    \$moduleinfo = new stdClass();
    \$moduleinfo->modulename = 'crucible';
    \$moduleinfo->modname = 'crucible';
    ...
    \$moduleinfo->intro = '<p>Crucible Moodle task grading test lab using Steamfitter scenario template ' . \$scenariotemplateid . '.</p>';
    \$moduleinfo->introformat = FORMAT_HTML;
    \$moduleinfo->showdescription = 0;
    \$moduleinfo->eventtemplateid = \$eventtemplateid;
    \$moduleinfo->vmapp = 1;
    \$moduleinfo->clock = 1;
    \$moduleinfo->extendevent = 1;
    \$moduleinfo->allowstudentinvites = 1;
    \$moduleinfo->extendinterval = 30;
    \$moduleinfo->timeopen = 0;
    \$moduleinfo->timeclose = 0;
    \$moduleinfo->grade = 100;
    \$moduleinfo->grademethod = 1;
    \$moduleinfo->contentlicense = 'unknown';
    \$moduleinfo->showcontentlicense = 0;
    \$moduleinfo->completion = COMPLETION_TRACKING_NONE;
    \$moduleinfo->availabilityconditionsjson = json_encode(['op' => '&', 'c' => [], 'showc' => []]);
    \$moduleinfo->groupmode = 0;
    \$moduleinfo->groupingid = 0;

    \$created = add_moduleinfo(\$moduleinfo, \$course);
    \$cm = get_coursemodule_from_id('crucible', \$created->coursemodule, \$courseid, false, MUST_EXIST);
    \$crucible = \$DB->get_record('crucible', ['id' => \$cm->instance], '*', MUST_EXIST);
    echo "Created Crucible Moodle activity: crucibleid={\$crucible->id} cmid={\$cm->id}\n";
}

\$auth = setup_system();
\$tasks = \$auth ? get_scenariotemplatetasks(\$auth, \$scenariotemplateid) : null;
if (!is_array(\$tasks) || empty(\$tasks)) {
    echo "No Steamfitter tasks returned for scenario template {\$scenariotemplateid}\n";
    exit(2);
}
...
foreach (\$tasks as \$task) {
    ...
    \$record = \$DB->get_record_sql(
        'SELECT * FROM {crucible_tasks} WHERE crucibleid = :crucibleid AND ' . \$DB->sql_compare_text('dispatchtaskid') . ' = ' . \$DB->sql_compare_text(':dispatchtaskid'),
        ['crucibleid' => \$crucible->id, 'dispatchtaskid' => \$task->id]
    );

    \$data = new stdClass();
    \$data->crucibleid = \$crucible->id;
    \$data->scenariotemplateid = \$scenariotemplateid;
    ...
    \$data->gradable = 1;
    \$data->multiple = 0;
    \$data->points = isset(\$task->score) ? (int)\$task->score : 0;
...
echo "Upserted {\$upserted} Moodle Crucible task definition rows\n";
echo "Activity URL: http://localhost:8081/mod/crucible/view.php?id={\$cm->id}\n";
PHP
}
...
# Remove Proxmox VMs. Scope:
#   managed (default) - only Crucible/TopoMojo-owned VMs: TopoMojo gamespace
#                       clones (PVE name contains "--", the "#" isolation
#                       separator, since "#" is illegal in Proxmox names) and
#                       the setup script's own base VMs, by known ID.
#   all               - every non-template VM on the host (destructive; intended
#                       only for a dedicated Crucible dev box).
cleanup_proxmox_vms() {
    local scope="${1:-managed}"
    ...
    # Base VMs/templates the setup script itself creates, by ID.
    local crucible_ids="103 105 106 9001 9002 9003 ${PUPPY_PROXMOX_ID} ${ALPINE_PROXMOX_ID} ${TINYCORE_PROXMOX_ID}"

    if [ "$scope" = "all" ]; then
        log_step "Stopping and removing ALL non-template VMs..."
    else
        log_step "Stopping and removing Crucible/TopoMojo-managed VMs..."
    fi

    ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$PROXMOX_USER@$PROXMOX_HOST" \
        "SCOPE='$scope' CRUCIBLE_IDS='$crucible_ids' bash -s" << 'CLEANVMS'
set -e

# Decide whether a given VM is in scope for deletion.
should_delete() {
    local vmid="$1" name="$2"
    if [ "$SCOPE" = "all" ]; then return 0; fi
    # TopoMojo gamespace clone (name "<template>--<isolationTag>").
    if printf '%s' "$name" | grep -q -- '--'; then return 0; fi
    # One of the setup script's own base VMs.
    if printf ' %s ' "$CRUCIBLE_IDS" | grep -qw "$vmid"; then return 0; fi
    return 1
}

for vmid in $(qm list | tail -n +2 | awk '{print $1}'); do
    # Templates are handled by ID below; never destroy them here.
    if qm config "$vmid" | grep -q "^template: 1"; then continue; fi
    name=$(qm config "$vmid" | awk -F': ' '/^name:/ {print $2; exit}')
    if should_delete "$vmid" "$name"; then
        echo "Deleting VM $vmid ($name)..."
        qm stop "$vmid" 2>/dev/null || true
        qm destroy "$vmid" --purge 1 || true
    else
        echo "Skipping unmanaged VM $vmid ($name)"
    fi
done
CLEANVMS

    log_step "Removing Crucible VM templates (105, 106, 9001, 9002, 9003)..."
...
phase2_vm_templates() {
    ...
    print_section "Phase 2/9: VM Template Creation"
...
phase3_wait_aspire() {
    print_section "Phase 3/9: Aspire Service Health Check"
...
phase4_topomojo_workspaces() {
    ...
    print_section "Phase 4/9: TopoMojo Workspaces"
...
phase6_player_views() {
    ...
    print_section "Phase 6/9: Player Views"
...
phase9_moodle_task_grading_lab() {
    ...
    log_success "Moodle task-grading lab ready"
}
...
mode_setup() {
    ...
    phase8_steamfitter_tasks || true
    phase9_moodle_task_grading_lab || true
    ...
    if [ -n "$token" ]; then
        ...
        local event_count=$(echo "$events" | jq -r '.[] | select(.name | test("Proxmox|Alloy Event|Moodle Task Grading")) | .id' | wc -l)
    ...
    token=$(get_keycloak_token "${KEYCLOAK_CLIENTS[steamfitter]}" 2>/dev/null)
    if [ -n "$token" ]; then
        local steamfitter_templates=$(curl -k -s -X GET "$STEAMFITTER_API_URL/scenarioTemplates" -H "Authorization: Bearer $token" 2>/dev/null || echo "[]")
        local steamfitter_template_id=$(echo "$steamfitter_templates" | jq -r '.[] | select(.name == "Moodle Task Grading Test Scenario") | .id' | head -1)
        if [ -n "$steamfitter_template_id" ]; then
            local steamfitter_tasks=$(curl -k -s -X GET "$STEAMFITTER_API_URL/scenarioTemplates/$steamfitter_template_id/Tasks" -H "Authorization: Bearer $token" 2>/dev/null || echo "[]")
            local steamfitter_task_count=$(echo "$steamfitter_tasks" | jq -r '.[] | select(.name | startswith("Moodle grading ")) | .id' | wc -l)
            echo "  Steamfitter Moodle Tasks: $steamfitter_task_count"
        fi
    fi

    local moodle_task_grading_cmid=""
    if docker ps --format '{{.Names}}' | grep -qx moodle; then
        local moodle_lab_count=$(docker exec moodle php -r 'define("CLI_SCRIPT", true); require_once("/var/www/html/config.php"); global $DB; echo $DB->count_records("crucible", ["eventtemplateid" => "74257398-e6ed-46e3-881a-8d7edacfddb9"]);' 2>/dev/null || echo 0)
        moodle_task_grading_cmid=$(docker exec moodle php -r 'define("CLI_SCRIPT", true); require_once("/var/www/html/config.php"); global $DB; $sql = "SELECT cm.id FROM {course_modules} cm JOIN {modules} m ON m.id = cm.module JOIN {crucible} c ON c.id = cm.instance WHERE m.name = ? AND c.eventtemplateid = ? ORDER BY cm.id DESC"; echo $DB->get_field_sql($sql, ["crucible", "74257398-e6ed-46e3-881a-8d7edacfddb9"]) ?: "";' 2>/dev/null || echo "")
        echo "  Moodle Task-Grading Labs: $moodle_lab_count"
    fi
    ...
    if [ -n "$moodle_task_grading_cmid" ]; then
        echo "  • Moodle Task-Grading Lab: http://localhost:8081/mod/crucible/view.php?id=$moodle_task_grading_cmid"
    fi
    echo ""
...
mode_cleanall() {
    ...
    cleanup_all
    cleanup_proxmox_vms all

    print_header "Complete Cleanup Done!"
}

mode_clean_vms() {
    print_header "Crucible Proxmox Managed-VM Cleanup"

    log_warning "This will delete Crucible/TopoMojo-managed VMs and templates (other VMs on the host are left alone)"
    read -p "Continue? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        log_info "Cleanup cancelled"
    ...
    cleanup_all
    cleanup_proxmox_vms managed

    print_header "Managed-VM Cleanup Done!"
}

mode_rebuild_alpine() {
    print_header "Crucible Proxmox Alpine Template Rebuild"

    load_config || true

    if [ -z "$PROXMOX_HOST" ]; then
        log_error "PROXMOX_HOST not set. Cannot rebuild Alpine template."
        exit 1
    fi

    log_warning "This will delete and recreate Proxmox template VM $ALPINE_PROXMOX_ID only"
    read -p "Continue? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        log_info "Alpine template rebuild cancelled"
        exit 0
    fi

    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY RUN] Would rebuild Alpine template VM $ALPINE_PROXMOX_ID"
        return 0
    fi

    ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$PROXMOX_USER@$PROXMOX_HOST" bash << VMEOF
set -e
if qm status $ALPINE_PROXMOX_ID >/dev/null 2>&1; then
    qm stop $ALPINE_PROXMOX_ID --skiplock 1 2>/dev/null || true
    qm destroy $ALPINE_PROXMOX_ID --purge 1
fi
VMEOF

    create_alpine_template

    log_success "Alpine template rebuilt"
}
...
Commands:
  ...
  clean-vms - Remove API resources + only Crucible/TopoMojo-managed Proxmox VMs
  cleanall  - Remove ALL resources including every non-template Proxmox VM
  rebuild-alpine - Recreate only the Alpine Proxmox template VM
