...
Each app's brand color is the default for both its top-bar background and its light-mode `primary`. The two are configured by separate settings (§4), so they can be changed independently, but they ship with the same value.
...
By default, light mode uses the brand pair from §2 unchanged:

- `--mat-sys-primary` = brand color (`AppLightModePrimaryHexColor`)
- `--mat-sys-on-primary` = text-on-brand color (`AppLightModePrimaryHexTextColor`)
- Top bar background and text = the same pair (`AppTopBarHexColor` / `AppTopBarHexTextColor`)

### 3b. Dark mode

The **top bar keeps the same colors in both modes.** The top-bar settings are not mode-specific. Its text sits on the top-bar color rather than on a dark surface, so the §2 pair stays compliant, and users see the same app identity in either mode.
...
- `on-primary` only colors text and icons drawn on a filled `primary` background, such as filled buttons, FABs, selected chips, and badges. It does not make dark-mode text black in general. Body text, headings, and labels on dark surfaces keep M3's light `on-surface` colors. Top-bar text keeps `AppTopBarHexTextColor`. Text buttons, links, and action icons use the dark-mode `primary` shade itself.
- If an app's brand color already passes all three checks in dark mode, it does **not** get a separate shade. Its dark-mode keys ship with the brand pair unchanged. The dark shade is added only when it is needed.

---

## 4. Settings contract

Colors are set through the app's `settings.json`. The top bar and each mode's `primary` pair have their own keys, and each key controls exactly one thing. The keys are:

| Setting | Meaning | Default |
|---|---|---|
| `AppTopBarHexColor` | Top-bar background, in both modes | §2 brand color |
| `AppTopBarHexTextColor` | Text/icon color on the top bar, in both modes | §2 text-on-brand color |
| `AppLightModePrimaryHexColor` | `primary` in light mode | §2 brand color |
| `AppLightModePrimaryHexTextColor` | `on-primary` in light mode | §2 text-on-brand color |
...
```json
{
  ...
  "AppLightModePrimaryHexColor": "#006B6D",
...
- The shipped `settings.json` for every app contains all six keys, filled in with the defaults from §2 and §3b. The defaults belong in the base settings file, so they stay in effect underneath any `settings.shared.json` / `settings.env.json` overlay an operator mounts.
- Each of the six values is resolved independently. The top-bar keys never affect `primary`, and the `primary` keys never affect the top bar.
- If a dark-mode key is missing, dark mode falls back to the matching light-mode key (a missing `AppDarkModePrimaryHexColor` falls back to `AppLightModePrimaryHexColor`, and a missing `AppDarkModePrimaryHexTextColor` falls back to `AppLightModePrimaryHexTextColor`). If a top-bar or light-mode key is missing, the app uses its hard-coded §2 value.
...
Operators may override any of the six settings, for example to match an organization's own branding.
...
- **The operator owns compliance for overridden colors.** An operator who changes a color must make sure the result still meets WCAG 2.1 AA in both modes, using the checks in §2 and §3b. Changing only the top-bar keys does not change `primary`, so rebranding an app usually means setting all three pairs: top bar, light-mode `primary`, and dark-mode `primary`. A color that works on the top bar or in light mode is often not compliant on dark surfaces.
- Operator-facing documentation for each app's color settings must say this: overriding colors moves accessibility responsibility to the operator, and rebranding usually means setting the top-bar, light-mode, and dark-mode keys together.
